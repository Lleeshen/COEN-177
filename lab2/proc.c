 ...
 */
  ...
  /* Declare extra variables for random scheduling */
  int qc;
  int chance;
  /* The 7th queue is the first queue that does not interfere with essential
   * processes
   */
  const int NUM = 7;
  ...
  /* Seed a hard-to-guess number to use in rand */
  srand(113);
  /* Iterate over the queues of different priority */
  for (qc=0; qc < NR_SCHED_QUEUES; qc++) {
    /* Use q instead of the actual priority later */
    q = qc;
    /* Make sure essential processes of the OS runs properly */
    if(qc >= NUM) {
        /* Each item has about 10% chance of being changed to a queue of different priority */
        chance = rand() % 31;
        if(chance < 3) {
            /* Randomize the priority process while making sure that the essential queues are not touched */
